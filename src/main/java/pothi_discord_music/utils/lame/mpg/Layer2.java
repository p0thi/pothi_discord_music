...
public class Layer2 {
  ...
  private static final double mulmul[] = {0.0, -2.0 / 3.0, 2.0 / 3.0,
      2.0 / 7.0, 2.0 / 15.0, 2.0 / 31.0, 2.0 / 63.0, 2.0 / 127.0,
      2.0 / 255.0, 2.0 / 511.0, 2.0 / 1023.0, 2.0 / 2047.0, 2.0 / 4095.0,
  ...
  private final int[] table[] = {null, null, null, grp_3tab, null, grp_5tab,
      null, null, null, grp_9tab};
  private final int base[][] = {{1, 0, 2,}, {17, 18, 0, 19, 20,},
      {21, 1, 22, 23, 0, 24, 25, 2, 26}};
  private final int tablen[] = {3, 5, 9};
  private final L2Tables.al_table2[] tables2[] = {L2Tables.alloc_0,
      L2Tables.alloc_1, L2Tables.alloc_2, L2Tables.alloc_3,
      L2Tables.alloc_4};
  ...
  private void II_step_two(final mpstr_tag mp, final int[] bit_alloc,
        ...
        if (ba != 0) {
          ...
          if ((d1 = fr.alloc[alloc2].d) < 0) {
            ...
            fraction[j][1][i] = ((float) ((int) common.getbits(mp,
                k) + d1)) * cm;
            fraction[j][2][i] = ((float) ((int) common.getbits(mp,
                k) + d1)) * cm;
          } else {
    ...
    for (i = jsbound; i < sblimit; i++, alloc1 += (1 << step)) {
      ...
      if (ba != 0) {
        ...
        fraction[0][0][i] = fraction[0][1][i] = fraction[0][2][i] = fraction[1][0][i] = fraction[1][1][i] = fraction[1][2][i] = 0.0f;
      }
      /*
			 * should we use individual scalefac for channel 2 or is the current
			 * way the right one , where we just copy channel 1 to channel 2 ??
			 * The current 'strange' thing is, that we throw away the scalefac
			 * values for the second channel ...!! . changed .. now we use the
			 * scalefac values of channel one !!
			 */
    }
  ...
  public <T> int do_layer2(final mpstr_tag mp, final float[] pcm_sample,
    ...
    int clip = 0;
    int i, j;
    float fraction[][][] = new float[2][4][MPG123.SBLIMIT]; /*
																 * pick_table
																 * clears unused
																 * subbands
																 */
    ...
    II_select_table(fr);
    fr.jsbound = (fr.mode == MPG123.MPG_MD_JOINT_STEREO) ? (fr.mode_ext << 2) + 4
        : fr.II_sblimit;
    ...
    for (i = 0; i < MPG123.SCALE_BLOCK; i++) {
      ...
      for (j = 0; j < 3; j++) {
        if (single >= 0) {
          clip += synth.synth_1to1_mono_ptr(mp, fraction[single][j],
              0, pcm_sample, pcm_point);
        } else {
          ProcessedBytes p1 = new ProcessedBytes();
          p1.pb = pcm_point.pb;
          clip += synth.synth_1to1_ptr(mp, fraction[0][j], 0, 0,
              pcm_sample, p1);
          clip += synth.synth_1to1_ptr(mp, fraction[1][j], 0, 1,
