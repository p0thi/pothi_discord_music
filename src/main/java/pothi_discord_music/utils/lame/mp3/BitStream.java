...
import pothi_discord_music.utils.lame.mpg.MPGLib;
...
public class BitStream {
  ...
  private void drain_into_ancillary(final LameGlobalFlags gfp,
    ...
    if (remainingBits >= 32) {
      final String versionStr = version.getLameShortVersion();
      if (remainingBits >= 32)
        for (i = 0; i < versionStr.length() && remainingBits >= 8; ++i) {
          remainingBits -= 8;
          putbits2(gfc, versionStr.charAt(i), 8);
        }
    }
  ...
  private void encodeSideInfo2(final LameGlobalFlags gfp,
    ...
    writeheader(gfc, (gfp.getMpegVersion()), 1);
    ...
    writeheader(gfc, (gfp.getMode().getNumMode()), 2);
