...
/* $Id: PsyModel.java,v 1.29 2012/03/12 15:58:57 kenchis Exp $ */
...
public class PsyModel {
  ...
  public final int L3psycho_anal_ns(final LameGlobalFlags gfp,
    ...
    if (gfp.getVBR() == VbrMode.vbr_off)
      pcfact = gfc.ResvMax == 0 ? 0f : ((float) gfc.ResvSize)
          / gfc.ResvMax * 0.5f;
    else if (gfp.getVBR() == VbrMode.vbr_rh || gfp.getVBR() == VbrMode.vbr_mtrh
    ...
    if (gfp.getMode() == MPEGMode.STEREO || gfp.getMode() == MPEGMode.JOINT_STEREO) {
  ...
  private void vbrpsy_attack_detection(final LameGlobalFlags gfp,
    ...
    int n_chn_out = gfc.channels_out;
		/* chn=2 and 3 = Mid and Side channels */
    int n_chn_psy = (gfp.getMode() == MPEGMode.JOINT_STEREO) ? 4 : n_chn_out;
		/* Don't copy the input buffer into a temporary buffer */
		/* unroll the loop 2 times */
  ...
  public final int L3psycho_anal_vbr(final LameGlobalFlags gfp,
    ...
    int uselongblock[] = new int[2];
		...
		/* chn=2 and 3 = Mid and Side channels */
    int n_chn_psy = (gfp.getMode() == MPEGMode.JOINT_STEREO) ? 4
        : gfc.channels_out;
  ...
  public final int psymodel_init(final LameGlobalFlags gfp) {
    ...
    final float sfreq = gfp.getOutSampleRate();
    ...
    switch (gfp.experimentalZ) {
      ...
      case 0:
        ...
        break;
      case 1:
        useOldS3 = (gfp.getVBR() == VbrMode.vbr_mtrh || gfp.getVBR() == VbrMode.vbr_mt) ? false
            : true;
        break;
      case 2:
        useOldS3 = false;
    ...
    if (gfp.ATHtype != -1) {
      ...
      float freq;
      final float freq_inc = (float) gfp.getOutSampleRate()
