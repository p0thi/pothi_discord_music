...
import pothi_discord.utils.lame.mp3.VBRTag;
import pothi_discord.utils.lame.mp3.VBRTagData;
...
public class Interface {
  ...
  MPGLib.buf addbuf(MPGLib.mpstr_tag mp, byte[] buf, int bufPos, int size) {
  ...
  int decodeMP3_clipchoice(MPGLib.mpstr_tag mp, byte[] in, int inPos, int isize,
                           float[] out, MPGLib.ProcessedBytes done, ISynth synth) {
  ...
  int decodeMP3(MPGLib.mpstr_tag mp, byte[] in, int bufferPos, int isize,
                float[] out, int osize, MPGLib.ProcessedBytes done) {
    if (osize < 2304) {
      System.err.printf(
    ...
    ISynth synth = new ISynth() {
      ...
      @Override
      public int synth_1to1_mono_ptr(MPGLib.mpstr_tag mp, float[] in, int inPos,
                                     float[] out, MPGLib.ProcessedBytes p) {
        return decode.synth1to1mono(mp, in, inPos, out, p);
      }

      @Override
      public int synth_1to1_ptr(MPGLib.mpstr_tag mp, float[] in, int inPos,
                                int i, float[] out, MPGLib.ProcessedBytes p) {
  ...
  int decodeMP3_unclipped(MPGLib.mpstr_tag mp, byte[] in, int bufferPos,
                          int isize, float[] out, int osize, MPGLib.ProcessedBytes done) {
		/*
  ...
  interface ISynth {
    int synth_1to1_mono_ptr(MPGLib.mpstr_tag mp, float[] in, int inPos,
                            float[] out, MPGLib.ProcessedBytes p);

    int synth_1to1_ptr(MPGLib.mpstr_tag mp, float[] in, int inPos, int i,
                       float[] out, MPGLib.ProcessedBytes p);
  }
}
