...
public class Layer1 {
  ...
  private void I_step_one(final MPGLib.mpstr_tag mp, final int balloc[],
  ...
  private void I_step_two(final MPGLib.mpstr_tag mp, final float fraction[][],
    ...
    if (fr.stereo == 2) {
      ...
      for (sample = 0, i = 0; i < jsbound; i++) {
        n = balloc[ba++];
        if (n != 0)
          fraction[0][f0++] = (float) (((-1) << n) + (smpb[sample++]) + 1)
              * common.muls[n + 1][scale_index[sca++]];
        else
          fraction[0][f0++] = 0.0f;
        n = balloc[ba++];
        if (n != 0)
          fraction[1][f1++] = (float) (((-1) << n) + (smpb[sample++]) + 1)
              * common.muls[n + 1][scale_index[sca++]];
        else
          fraction[1][f1++] = 0.0f;
      }
  ...
  public int do_layer1(final MPGLib.mpstr_tag mp, final float[] pcm_sample,
                       final MPGLib.ProcessedBytes pcm_point) {
    int clip = 0;
    ...
    for (i = 0; i < MPG123.SCALE_BLOCK; i++) {
      ...
      if (single >= 0) {
        clip += decode.synth1to1mono(mp, fraction[single], 0,
            pcm_sample, pcm_point);
      } else {
        MPGLib.ProcessedBytes p1 = new MPGLib.ProcessedBytes();
        p1.pb = pcm_point.pb;
        clip += decode
            .synth_1to1(mp, fraction[0], 0, 0, pcm_sample, p1);
        clip += decode.synth_1to1(mp, fraction[1], 0, 1, pcm_sample,
            pcm_point);
      }
    }
