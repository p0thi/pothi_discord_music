...
public class VBRTag {
  ...
  public final void addVbrFrame(final LameGlobalFlags gfp) {
    ...
    int kbps = Tables.bitrate_table[gfp.getMpegVersion()][gfc.bitrate_index];
   ...
   *
   * @param buf    header to write the integer into
   * @param bufPos offset into the header
   ...
   *
   * @param gfp             global flags
   * @param musicLength     audio length
  ...
  private int putLameVBR(final LameGlobalFlags gfp, final int musicLength,
    ...
    int bytesWritten = 0;
		...
		/* encoder delay */
    int encDelay = gfp.getEncoderDelay();
		/* encoder padding */
    int encPadding = gfp.encoder_padding;

		/* recall: gfp.VBR_q is for example set by the switch -V */
		/* gfp.quality by -q, -h, -f, etc */
    int quality = (100 - 10 * gfp.getVBRQuality() - gfp.getQuality());

    final String versionStr = version.getLameVeryShortVersion();
    int vbr;
    int revision = 0x00;
    int revMethod;
    ...
    if (gfp.getVBR().ordinal() < vbrTypeTranslator.length)
      vbr = vbrTypeTranslator[gfp.getVBR().ordinal()];
    else
      vbr = 0x00; // unknown
    ...
    if (gfp.short_blocks == ShortBlock.short_block_forced
        ...
        || (gfp.scale_left > gfp.scale_right)
        || (gfp.disable_reservoir && gfp.getBitRate() < 320) || gfp.noATH
        || gfp.ATHonly || (athType == 0) || gfp.getInSampleRate() <= 32000)
      nonOptimal = 1;
    ...
    for (int j = 0; j < 9; j++) {
      streamBuffer[streamBufferPos + bytesWritten + j] = (byte) versionStr
          .charAt(j);
    }
    bytesWritten += 9;
   ...
   *
   * @param gfp    global flags
   * @param stream stream to add the VBR tag to
   * @return 0 (OK), -1 else
